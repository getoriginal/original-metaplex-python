...
from original_metaplex_python.token_metadata.generated.instructions.create_metadata_account_v2 import (
    create_metadata_account_v2,
)
from original_metaplex_python.token_metadata.generated.program_id import PROGRAM_ID


def test_create_metadata_account_v2():
    accounts = {
    ...
    instruction = create_metadata_account_v2(accounts=accounts, program_id=PROGRAM_ID)
    assert len(instruction.accounts) == 7
    assert instruction.program_id == PROGRAM_ID


def test_create_metadata_account_v2_with_remaining_accounts():
    ...
    instruction = create_metadata_account_v2(
        accounts=accounts,
