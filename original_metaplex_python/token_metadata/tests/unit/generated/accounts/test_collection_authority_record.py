...
from original_metaplex_python.token_metadata.generated.accounts.collection_authority_record import (
    PROGRAM_ID,
...
@pytest.mark.asyncio
async def test_fetch_wrong_program_id(mocker: MockerFixture):
    mock_client = mocker.Mock(spec=AsyncClient)
    ...
    with pytest.raises(ValueError, match="Account does not belong to this program"):
        await CollectionAuthorityRecord.fetch(mock_client, address)
...
async def test_fetch_multiple_success(mocker: MockerFixture):
    ...
    mock_get_multiple_accounts = mocker.patch(
        "original_metaplex_python.token_metadata.generated.accounts.collection_authority_record.get_multiple_accounts",
...
@pytest.mark.asyncio
async def test_fetch_multiple_wrong_program_id(mocker: MockerFixture):
    mock_records = [
        CollectionAuthorityRecord(
            key=CollectionAuthorityRecordKey(),
            bump=1,
    ...
    # First account info has the correct program ID, second has a wrong program ID
    fake_account_infos = [
        AccountInfo(
            data=b"correctly_encoded_data_1",
            owner=PROGRAM_ID,
    ...
    with pytest.raises(ValueError, match="Account does not belong to this program"):
        await CollectionAuthorityRecord.fetch_multiple(
            mock_client, addresses, commitment=None
        )
...
def test_decode(mocker: MockerFixture):
    ...
    mocker.patch(
        "original_metaplex_python.token_metadata.generated.accounts.collection_authority_record.CollectionAuthorityRecord.layout.parse",
